/**
 * Tax estimation utility.
 *
 * Uses 2024 federal brackets (single filer), FICA, and simplified state
 * effective rates. Good enough for a planning estimate — not tax advice.
 */

// ── ZIP prefix → state abbreviation ──────────────────────────────────
// First 3 digits of a US ZIP code map to a state / territory.
const ZIP_PREFIX_TO_STATE: Record<string, string> = {
  ...
  // New York
  '005': 'NY','100': 'NY','101': 'NY','102': 'NY','103': 'NY','104': 'NY','105': 'NY','106': 'NY','107': 'NY','108': 'NY','109': 'NY','110': 'NY','111': 'NY','112': 'NY','113': 'NY','114': 'NY','115': 'NY','116': 'NY','117': 'NY','118': 'NY','119': 'NY','120': 'NY','121': 'NY','122': 'NY','123': 'NY','124': 'NY','125': 'NY','126': 'NY','127': 'NY','128': 'NY','129': 'NY','130': 'NY','131': 'NY','132': 'NY','133': 'NY','134': 'NY','135': 'NY','136': 'NY','137': 'NY','138': 'NY','139': 'NY','140': 'NY','141': 'NY','142': 'NY','143': 'NY','144': 'NY','145': 'NY','146': 'NY','147': 'NY','148': 'NY','149': 'NY',
  // North Carolina
...
function calcFica(grossIncome: number): number {
...
// ── Simplified state effective tax rates (2024 estimates) ────────────
// These are rough effective rates for a median-ish income (~$70-80k).
// States with 0 = no income tax.
const STATE_TAX_RATES: Record<string, number> = {
  AL: 0.040, AK: 0, AZ: 0.025, AR: 0.044, CA: 0.066, CO: 0.044,
  CT: 0.050, DE: 0.048, DC: 0.060, FL: 0, GA: 0.049, HI: 0.064,
  ID: 0.058, IL: 0.0495, IN: 0.0315, IA: 0.044, KS: 0.046, KY: 0.040,
  LA: 0.030, ME: 0.058, MD: 0.050, MA: 0.050, MI: 0.0425, MN: 0.062,
  MS: 0.047, MO: 0.048, MT: 0.059, NE: 0.056, NV: 0, NH: 0,
  NJ: 0.055, NM: 0.041, NY: 0.060, NC: 0.045, ND: 0.018, OH: 0.035,
  OK: 0.038, OR: 0.080, PA: 0.0307, RI: 0.047, SC: 0.050, SD: 0,
  TN: 0, TX: 0, UT: 0.0465, VT: 0.060, VA: 0.050, WA: 0, WV: 0.051,
  WI: 0.053, WY: 0,
}
...
export function estimateTaxes(annualGrossIncome: number, zipCode: string): TaxEstimate {
  if (annualGrossIncome <= 0) {
    return { state: null, stateName: null, federal: 0, stateTax: 0, fica: 0, totalTax: 0, effectiveRate: 0, annualTakeHome: 0, monthlyTakeHome: 0 }
  }
  ...
  const federal = calcFederalTax(taxableIncome)

  // FICA (applied to gross, not taxable)
  const fica = calcFica(annualGrossIncome)

  // State
  const stateRate = state ? (STATE_TAX_RATES[state] ?? 0) : 0
  const stateTax = annualGrossIncome * stateRate

  const totalTax = federal + stateTax + fica
  const effectiveRate = totalTax / annualGrossIncome
  const annualTakeHome = annualGrossIncome - totalTax
  const monthlyTakeHome = annualTakeHome / 12

  return {
    state,
    stateName: state ? getStateName(state) : null,
